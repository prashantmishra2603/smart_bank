...
const DepositPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await API.post(`/accounts/${accountId}/deposit`, { amount: Number(amount) });
      setLastTxn(res.data);
      setAmount('');
      ...
      showToast(err.response?.data?.error || err.response?.data?.message || 'Deposit failed', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '680px' }}>
      <div>
        <h1 style={{ fontSize: '1.5rem', fontWeight: '700', letterSpacing: '-0.5px' }}>Deposit Funds</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginTop: '4px' }}>Add money to your bank account</p>
      </div>
      ...
      {selectedAccount && (
        <div style={{
          ...
          <div style={{ fontSize: '12px', opacity: 0.8, marginBottom: '4px' }}>Current Balance</div>
      ...
      <div className="glass-card" style={{ padding: '28px' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              {quickAmounts.map(q => (
                <button
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${String(amount) === String(q) ? 'var(--brand)' : 'var(--border)'}`,
                    background: String(amount) === String(q) ? 'var(--brand-light)' : 'white',
                    color: String(amount) === String(q) ? 'var(--brand)' : 'var(--text-secondary)',
          ...
          <button type="submit" className="btn-primary" disabled={loading || accounts.length === 0} style={{ padding: '12px 28px', fontSize: '15px' }}>
      ...
      {lastTxn && (
        <div className="glass-card" style={{ padding: '20px', border: '1px solid #6ee7b7' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
            ...
            <span style={{ fontWeight: '700', color: 'var(--green)' }}>Last Transaction</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '14px' }}>
            ...
            <div>
              ...
              <div style={{ fontWeight: '700', color: 'var(--green)' }}>+₹{Number(lastTxn.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
            </div>
            <div>
